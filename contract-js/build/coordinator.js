...
BigInt(10000000000000000000000); //0.01 NEAR in yoctoNEAR

BigInt(1000000000000000000000); //0.001 NEAR in yoctoNEAR

const MIN_COORDINATOR_STORAGE = POINT_ONE_NEAR; //0.1 NEAR

const MIN_NODE_STORAGE = POINT_ONE_NEAR; //0.01 NEAR
...
let Coordinator = (_dec = NearBindgen({}), _dec2 = initialize({}), _dec3 = call({
  ...
  payableFunction: true
}), _dec5 = call({}), _dec6 = view({}), _dec7 = view({}), _dec8 = call({}), _dec(_class = (_class2 = class Coordinator {
  //Currently, it cost approximately 1 Ⓝ to store 100kb of data. When registering a node, we need to collect for storage.
  nodes = new UnorderedMap('nodes');
  bounties = new UnorderedMap('bounties');
  oracleFee = 0.1;

  init({}) {
    this.signerAccountId = signerAccountId();
    this.oracleAccountId = currentAccountId();
  }
  ...
  createAndRegisterNode({
    ...
    log("Starting here");
    assert(attachedDeposit() > MIN_STORAGE, "You must pay the registration fee to register a node");
    log(attachedDeposit());
    log(signerAccountId());
  ...
  createBounty({
    ...
    attachedDeposit(); // const requestingAccount //Do we need to store this? We don't do we?
...
}, (_applyDecoratedDescriptor(_class2.prototype, "init", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "init"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "createAndRegisterNode", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "createAndRegisterNode"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "createBounty", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "createBounty"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "distributeBounties", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "distributeBounties"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getNode", [_dec6], Object.getOwnPropertyDescriptor(_class2.prototype, "getNode"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getRegisteredNodeCount", [_dec7], Object.getOwnPropertyDescriptor(_class2.prototype, "getRegisteredNodeCount"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "electNodes", [_dec8], Object.getOwnPropertyDescriptor(_class2.prototype, "electNodes"), _class2.prototype)), _class2)) || _class); //Ideas for supported software:
// cURL, Go, Node, Python, Bash, venv,

function electNodes() {
  let _state = Coordinator._getState();

  if (!_state && Coordinator._requireInit()) {
    throw new Error("Contract must be initialized");
  }

  let _contract = Coordinator._create();

  if (_state) {
    Coordinator._reconstruct(_contract, _state);
  }

  let _args = Coordinator._getArgs();

  let _result = _contract.electNodes(_args);

  Coordinator._saveToStorage(_contract);

  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Coordinator._serialize(_result));
}
function getRegisteredNodeCount() {
  let _state = Coordinator._getState();
  ...
  let _result = _contract.getRegisteredNodeCount(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Coordinator._serialize(_result));
}
function getNode() {
  let _state = Coordinator._getState();
  ...
  let _result = _contract.getNode(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Coordinator._serialize(_result));
}
function distributeBounties() {
  let _state = Coordinator._getState();
  ...
  let _result = _contract.distributeBounties(_args);
...
function createBounty() {
  ...
  let _result = _contract.createBounty(_args);

  Coordinator._saveToStorage(_contract);

  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Coordinator._serialize(_result));
}
function createAndRegisterNode() {
  let _state = Coordinator._getState();
  ...
  let _result = _contract.createAndRegisterNode(_args);
...
export { MIN_COORDINATOR_STORAGE, MIN_NODE_STORAGE, MIN_STORAGE, createAndRegisterNode, createBounty, distributeBounties, electNodes, getNode, getRegisteredNodeCount, init };
