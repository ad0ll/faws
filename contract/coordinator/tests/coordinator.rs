...
pub async fn complete_bounty(coordinator_contract: Contract, bounty: &Bounty, accounts: HashMap<AccountId, Account>, nodes: &HashMap<AccountId, Node>, n_succeeded: u64, n_failed: u64) -> anyhow::Result<()> {
    ...
    async fn process_node(coordinator_contract: &Contract, bounty: &Bounty, accounts: &HashMap<AccountId, Account>, nodes: &HashMap<AccountId, Node>, node_id: AccountId, solution: String, message: String, status: NodeResponseStatus) -> anyhow::Result<NodeResponse> {
        let node = nodes.get(&node_id).unwrap_or_else(|| panic!("failed to get node {} from nodes", &node_id));
        ...
        let node_response = post_answer(coordinator_contract.clone(), account.clone(), node.id.clone(), bounty.id.clone(), solution, message, status).await?;
        return Ok(node_response);
    }

    let bounty_is_complete = |bounty: &Bounty| -> bool {
        return bounty.status == BountyStatus::Cancelled ||
            (bounty.successful_nodes.len() >= bounty.min_nodes && bounty.status == BountyStatus::Success)
            || (bounty.failed_nodes.len() >= bounty.min_nodes && bounty.status == BountyStatus::Failed);
    };
    ...
    while curr_idx < n_succeeded {
        ...
        let nr = process_node(&coordinator_contract, &bounty, &accounts, &nodes, node_id.clone(), "42".to_string(), format!("SUCCESS node {} idx {}", node_id, curr_idx), NodeResponseStatus::SUCCESS).await;
    ...
    while curr_idx < n_succeeded + n_failed {
        let node_id = bounty.elected_nodes[curr_idx as usize].clone();
        let nr = process_node(&coordinator_contract, &bounty, &accounts, &nodes, node_id.clone(), "244".to_string(), format!("FAILED node {} idx {}", node_id, curr_idx), NodeResponseStatus::FAILURE).await;
...
async fn post_answer(coordinator_contract: Contract, account: Account, node_id: AccountId, bounty_id: AccountId, answer: String, message: String, status: NodeResponseStatus) -> anyhow::Result<NodeResponse> {
...
async fn should_post_answer(coordinator_contract: Contract, account: Account, bounty_id: AccountId, node_id: AccountId) -> anyhow::Result<bool> {
...
async fn test_bounty_full_lifecycle() -> anyhow::Result<()> {
    ...
    let min_nodes = 2;
    let max_nodes = 3;
    assert!(min_nodes + max_nodes <= num_accounts, "failed to create bounty, min_nodes + max_nodes must be less than or equal to num_accounts");
    let worker = near_workspaces::sandbox().await?;
    let coordinator_contract = setup_coordinator(worker.clone()).await?;
    ...
    println!("bounty: {} {} {} {}", hot_bounty.successful_nodes.len(), hot_bounty.failed_nodes.len(), hot_bounty.min_nodes, hot_bounty.answers.len());
    assert_eq!(hot_bounty.status, BountyStatus::Success, "bounty should be completed");
    assert!(hot_bounty.answers.len() >= min_nodes, "bounty should have at least min_nodes answers");
    assert!(hot_bounty.successful_nodes.len() >= min_nodes, "bounty should have at least min_nodes responses");
    ...
    let hot_bounty = get_bounty(&coordinator_contract, bounty.id.clone()).await?;
    assert_eq!(hot_bounty.id, bounty.id.clone(), "bounty should exist");
    assert_eq!(hot_bounty.status, BountyStatus::Failed, "bounty should be completed");
    assert!(hot_bounty.answers.len() >= min_nodes + 1, "bounty should have at least min_nodes answers");
    assert!(hot_bounty.failed_nodes.len() >= min_nodes, "bounty should have at least min_nodes failed responses");
