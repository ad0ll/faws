...
REFERENCE_BOUNTY=0-255781000.bounty.dev-1666506475610-88274883199945
...
if [[ -n $REDEPLOY_CONTRACT ]]; then #If defined then...
  ...
  local_near call "$CONTRACT_NAME" register_node '{"name": "node1", "allow_network": true, "allow_gpu": true}' --accountId="$ACCOUNT"
  local_near call "$CONTRACT_NAME" register_node '{"name": "node2", "allow_network": false, "allow_gpu": true}' --accountId="$ACCOUNT"
  local_near call "$CONTRACT_NAME" register_node '{"name": "node3", "allow_network": true, "allow_gpu": false}' --accountId="$ACCOUNT"
