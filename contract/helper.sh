...
if [[ -n $REDEPLOY_CONTRACT ]]; then #If defined then...
  ...
  #  local_near deploy faws-dev.testnet --wasmFile target/wasm32-unknown-unknown/release/coordinator.wasm
...
local_near call "$CONTRACT_NAME" reelect_nodes --accountId="$ACCOUNT" "{\"bounty_id\": \"$REFERENCE_BOUNTY\"}"
