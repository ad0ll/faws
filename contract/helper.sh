...
if [[ -n $REDEPLOY_CONTRACT ]]; then #If defined then...
  ...
  #local_near call "$CONTRACT_NAME" register_node '{"name": "node4", "allow_network": true, "allow_gpu": false}' --deposit 1 --accountId="$ACCOUNT"
  #local_near call "$CONTRACT_NAME" register_node '{"name": "node5", "allow_network": true, "allow_gpu": false}' --deposit 1 --accountId="$ACCOUNT"
fi
source_neardev
